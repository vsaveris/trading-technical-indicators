...
class TestDetrendedPriceOscillator(unittest.TestCase, TestIndicatorsCommon):

    indicator = tti.indicators.DetrendedPriceOscillator

    ti_data_rows = [0, 1, 2]

    df = pd.read_csv('./data/sample_data.csv', parse_dates=True, index_col=0)

    indicator_input_arguments = {'period': 6}
