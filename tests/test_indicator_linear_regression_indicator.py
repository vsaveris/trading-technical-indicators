...
File name: test_indicator_linear_regression_indicator.py
    tti.indicators package, _linear_regression_indicator.py module unit tests.
"""
...
class TestLinearRegressionIndicator(unittest.TestCase):
    ...
    def test_input_data_wrong_type(self):
        with self.assertRaises(TypeError):
            LinearRegressionIndicator('NO_DF')
    ...
    def test_input_data_required_column_close_missing(self):
        ...
        with self.assertRaises(ValueError):
            LinearRegressionIndicator(pd.DataFrame(df.drop(columns=['close'])))
    ...
    def test_input_data_empty(self):
        ...
        with self.assertRaises(ValueError):
            LinearRegressionIndicator(
                pd.DataFrame(df[df.index >= '2032-01-01']))
    ...
    def test_period_parameter_wrong_type(self):
        ...
        with self.assertRaises(WrongTypeForInputParameter):
            LinearRegressionIndicator(df, period='30')
    ...
    def test_period_parameter_wrong_value(self):
        ...
        with self.assertRaises(WrongValueForInputParameter):
            LinearRegressionIndicator(df, period=0)
    ...
    def test_fill_missing_values_parameter_wrong_type(self):
        ...
        with self.assertRaises(WrongTypeForInputParameter):
            LinearRegressionIndicator(df, period=200, fill_missing_values=1)
    ...
    def test_fill_missing_values_is_true(self):
        ...
        df_result = LinearRegressionIndicator(df, period=20,
    ...
    def test_fill_missing_values_is_default_true(self):
        ...
        df_result = LinearRegressionIndicator(df, period=20)._input_data
    ...
    def test_validate_indicator_one_row(self):
        ...
        with self.assertRaises(NotEnoughInputData):
            LinearRegressionIndicator(df[df.index == '2000-02-01'])
    ...
    def test_validate_indicator_less_than_required_data(self):
        ...
        with self.assertRaises(NotEnoughInputData):
            LinearRegressionIndicator(df.iloc[:13], period=14)
    ...
    def test_validate_indicator_full_data(self):
        ...
        df_expected_result = pd.read_csv(
            './data/test_linear_regression_indicator_on_sample_data.csv',
            parse_dates=True,
            index_col=0).round(4)

        df_result = LinearRegressionIndicator(df)._ti_data
    ...
    def test_getTiValue_latest(self):
        ...
        self.assertEqual(list(df_expected_result.iloc[-1]),
                         LinearRegressionIndicator(df).getTiValue())

    def test_getTiSignal(self):
        df = pd.read_csv('./data/sample_data.csv', parse_dates=True,
                         index_col=0)

        self.assertIn(LinearRegressionIndicator(df).getTiSignal(),
                      [('buy', -1), ('hold', 0), ('sell', 1)])
