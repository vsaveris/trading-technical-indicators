...
class TestIndicatorsCommon(ABC):
        ...
        for missing_column in self.required_input_data_columns:
            with self.subTest(missing_column=missing_column):
                with self.assertRaises(ValueError):
                    self.indicator(pd.DataFrame(df.drop(columns=[missing_column])))
    ...
    def test_argument_fill_missing_values_is_false(self):
        ...
        df_expected_result = pd.read_csv(
            "./data/missing_values_data_sorted.csv",
            parse_dates=True,
            date_format="%Y-%m-%d",
            index_col=0,
        )[self.required_input_data_columns].round(self.precision)
    ...
    def test_argument_fill_missing_values_is_default_true(self):
        ...
        df_result = self.indicator(df, **self.indicator_input_arguments)._input_data[
            self.required_input_data_columns
        ]
    ...
    def test_validate_indicator_exactly_required_input_data(self):
        ...
        self.indicator(
            df.iloc[: self.indicator_minimum_required_data], **self.indicator_input_arguments
        )
    ...
    def test_validate_indicator_full_data(self):
        ...
        df_expected_result = pd.read_csv(
            self.indicator_test_data_file_name,
            parse_dates=True,
            date_format="%Y-%m-%d",
            index_col=0,
        ).round(self.precision)

        df_result = self.indicator(df, **self.indicator_input_arguments)._ti_data

        pd.testing.assert_frame_equal(df_expected_result, df_result, check_dtype=False)
    ...
    def test_getTiValue_specific(self):
        ...
        self.assertEqual(
            ...
            self.indicator(df, **self.indicator_input_arguments).getTiValue("2009-10-19"),
        )
    ...
    def test_getTiValue_latest(self):
        ...
        actual_result = self.indicator(df, **self.indicator_input_arguments).getTiValue()
    ...
    def test_getTiSignal(self):
        ...
        self.assertIn(
            self.indicator(df, **self.indicator_input_arguments).getTiSignal(),
            [("buy", -1), ("hold", 0), ("sell", 1)],
        )
    ...
    def test_simulation_deprecated(self):
        ...
        with self.assertRaises(TtiPackageDeprecatedMethod):
            self.indicator(
                df[df.index >= "2011-09-12"], **self.indicator_input_arguments
            ).runSimulation(close_values=df[df.index >= "2011-09-12"])
    ...
    def test_getTiSimulation(self):
        ...
        simulation_data, statistics, graph = ti.getTiSimulation(df[["close"]])

        if (
            str(self.indicator)
            == "<class 'tti.indicators._detrended_price_oscillator." + "DetrendedPriceOscillator'>"
        ):
        ...
        self.assertEqual(any(np.isnan(val) for val in statistics.values()), False)
        ...
        graph.savefig(
            "./figures/trading_simulation_graph_" + str(self.indicator).split(".")[-1][:-2] + ".png"
        )
        plt.close("all")
