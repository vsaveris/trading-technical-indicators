...
class TestDirectionalMovementIndex(unittest.TestCase):
    ...
    def test_input_data_required_column_close_missing(self):
        ...
        with self.assertRaises(ValueError):
            DirectionalMovementIndex(pd.DataFrame(df.drop(columns=['close'])))
    ...
    def test_input_data_required_column_volume_missing(self):
        ...
        with self.assertRaises(ValueError):
            DirectionalMovementIndex(pd.DataFrame(df.drop(columns=['volume'])))
    ...
    def test_input_data_empty(self):
        ...
        with self.assertRaises(ValueError):
            DirectionalMovementIndex(pd.DataFrame(df[df.index >=
                                                     '2032-01-01']))
    ...
    def test_fill_missing_values_is_true(self):
        ...
        df_result = DirectionalMovementIndex(df, fill_missing_values=True)\
            ._input_data
    ...
    def test_fill_missing_values_is_false(self):
        ...
        df_result = DirectionalMovementIndex(df, fill_missing_values=False)\
            ._input_data
    ...
    def test_validate_indicator_one_row(self):
        ...
        df_result = DirectionalMovementIndex(df[df.index == '2000-02-01'])\
            ._ti_data
    ...
    def test_getTiData(self):
        ...
        pd.testing.assert_frame_equal(df_expected_result,
                                      DirectionalMovementIndex(df).getTiData())
    ...
    def test_getTiValue_specific(self):
        ...
        self.assertEqual(df_expected_result.loc['2000-04-25', 'OBV'],
                         DirectionalMovementIndex(df).getTiValue('2000-04-25'))
