...
File name: test_indicator_linear_regression_slope.py
    tti.indicators package, _linear_regression_slope.py module unit tests.
"""
...
class TestLinearRegressionSlope(unittest.TestCase):
    ...
    def test_input_data_required_column_close_missing(self):
        ...
        with self.assertRaises(ValueError):
            LinearRegressionSlope(pd.DataFrame(df.drop(columns=['close'])))
    ...
    def test_input_data_empty(self):
        ...
        with self.assertRaises(ValueError):
            LinearRegressionSlope(pd.DataFrame(df[df.index >= '2032-01-01']))
    ...
    def test_period_parameter_wrong_type(self):
        ...
        with self.assertRaises(WrongTypeForInputParameter):
            LinearRegressionSlope(df, period='30')
    ...
    def test_period_parameter_wrong_value(self):
        ...
        with self.assertRaises(WrongValueForInputParameter):
            LinearRegressionSlope(df, period=0)
    ...
    def test_fill_missing_values_parameter_wrong_type(self):
        ...
        with self.assertRaises(WrongTypeForInputParameter):
            LinearRegressionSlope(df, period=200, fill_missing_values=1)
    ...
    def test_fill_missing_values_is_true(self):
        ...
        df_result = LinearRegressionSlope(df, period=20,
    ...
    def test_fill_missing_values_is_default_true(self):
        ...
        df_result = LinearRegressionSlope(df, period=20)._input_data
    ...
    def test_validate_indicator_one_row(self):
        ...
        with self.assertRaises(NotEnoughInputData):
            LinearRegressionSlope(df[df.index == '2000-02-01'])
    ...
    def test_validate_indicator_less_than_required_data(self):
        ...
        with self.assertRaises(NotEnoughInputData):
            LinearRegressionSlope(df.iloc[:13], period=14)
    ...
    def test_validate_indicator_full_data(self):
        ...
        df_expected_result = pd.read_csv(
            './data/test_linear_regression_slope_on_sample_data.csv',
            parse_dates=True,
            index_col=0).round(4)

        df_result = LinearRegressionSlope(df)._ti_data
    ...
    def test_getTiGraph(self):
        ...
        obv = LinearRegressionSlope(df)

        # Needs manual check of the produced graph
        self.assertEqual(obv.getTiGraph(), plt)

        obv.getTiGraph().savefig('./figures/test_linear_regression_slope.png')
        plt.close('all')
    ...
    def test_getTiValue_latest(self):
        ...
        self.assertEqual(list(df_expected_result.iloc[-1]),
                         LinearRegressionSlope(df).getTiValue())

    def test_getTiSignal(self):
        df = pd.read_csv('./data/sample_data.csv', parse_dates=True,
                         index_col=0)

        self.assertIn(LinearRegressionSlope(df).getTiSignal(),
                      [('buy', -1), ('hold', 0), ('sell', 1)])
