...
class TestValidateInputData(unittest.TestCase):
    ...
    def test_input_data_is_not_dataframe(self):
        with self.assertRaises(WrongTypeForInputParameter):
            dv.validateInputData(input_data="No_DF", required_columns=["C1"], indicator_name="I")
    ...
    def test_input_data_index_is_not_date(self):
        ...
        with self.assertRaises(TypeError):
            dv.validateInputData(input_data=df, required_columns=["close"], indicator_name="I")
    ...
    def test_input_data_dataframe_is_empty(self):
        ...
        with self.assertRaises(ValueError):
            dv.validateInputData(
                input_data=df[df.index >= "2032-01-01"],
    ...
    def test_input_data_no_numeric_columns(self):
        ...
        df = df.astype("str")
        df.at[df.index[0], "close"] = "no-numeric"
    ...
    def test_input_data_missing_required_columns(self):
        ...
        with self.assertRaises(ValueError):
            dv.validateInputData(input_data=df, required_columns=["not_exist"], indicator_name="I")
    ...
    def test_input_data_not_required_columns_removed(self):
        ...
        df_result = dv.validateInputData(
            input_data=df, required_columns=["close"], indicator_name="I"
        )
    ...
    def test_input_data_already_sorted(self):
        df = pd.read_csv(
            "./data/sample_data_sorted.csv", parse_dates=True, date_format="%Y-%m-%d", index_col=0
        )
