...
File name: test_indicator_double_exponential_moving_average.py
    tti.indicators package, _double_exponential_moving_average.py module unit 
    tests.
"""
...
class TestDoubleExponentialMovingAverage(unittest.TestCase):
    ...
    def test_input_data_wrong_type(self):
        with self.assertRaises(TypeError):
            DoubleExponentialMovingAverage('NO_DF')
    ...
    def test_input_data_wrong_index_type(self):
        ...
        with self.assertRaises(TypeError):
            DoubleExponentialMovingAverage(df)
    ...
    def test_input_data_required_column_close_missing(self):
        ...
        with self.assertRaises(ValueError):
            DoubleExponentialMovingAverage(
                pd.DataFrame(df.drop(columns=['close'])))
    ...
    def test_input_data_empty(self):
        ...
        with self.assertRaises(ValueError):
            DoubleExponentialMovingAverage(pd.DataFrame(
    ...
    def test_period_parameter_wrong_type(self):
        ...
        with self.assertRaises(WrongTypeForInputParameter):
            DoubleExponentialMovingAverage(df, period='30')
    ...
    def test_period_parameter_wrong_value(self):
        ...
        with self.assertRaises(WrongValueForInputParameter):
            DoubleExponentialMovingAverage(df, period=0)
    ...
    def test_fill_missing_values_parameter_wrong_type(self):
        ...
        with self.assertRaises(WrongTypeForInputParameter):
            DoubleExponentialMovingAverage(df, period=200,
    ...
    def test_fill_missing_values_is_true(self):
        ...
        df_result = DoubleExponentialMovingAverage(df, period=20,
    ...
    def test_fill_missing_values_is_default_true(self):
        ...
        df_result = DoubleExponentialMovingAverage(df, period=20)._input_data
    ...
    def test_validate_indicator_one_row(self):
        ...
        with self.assertRaises(NotEnoughInputData):
            DoubleExponentialMovingAverage(df[df.index == '2000-02-01'])
    ...
    def test_validate_indicator_less_than_required_data(self):
        ...
        with self.assertRaises(NotEnoughInputData):
            DoubleExponentialMovingAverage(df.iloc[:13], period=14)
    ...
    def test_validate_indicator_full_data(self):
        ...
        df_expected_result = pd.read_csv(
            './data/test_double_exponential_moving_average_on_sample_data.csv',
            parse_dates=True,
            index_col=0).round(4)

        df_result = DoubleExponentialMovingAverage(df)._ti_data
    ...
    def test_getTiValue_latest(self):
        ...
        self.assertEqual(list(df_expected_result.iloc[-1]),
                         DoubleExponentialMovingAverage(df).getTiValue())

    def test_getTiSignal(self):
        df = pd.read_csv('./data/sample_data.csv', parse_dates=True,
                         index_col=0)

        self.assertIn(DoubleExponentialMovingAverage(df).getTiSignal(),
                      [('buy', -1), ('hold', 0), ('sell', 1)])
