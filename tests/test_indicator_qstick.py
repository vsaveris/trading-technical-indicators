...
File name: test_indicator_qstick.py
    tti.indicators package, _qstick.py module unit tests.
"""
...
class TestQstick(unittest.TestCase, TestIndicatorsCommon):

    indicator = tti.indicators.Qstick

    ti_data_rows = [0, 3, 4]

    df = pd.read_csv('./data/sample_data.csv', parse_dates=True, index_col=0)

    indicator_input_arguments = {'period': 8}
    ...
    required_input_data_columns = ["open", "close"]
