...
File name: test_indicator_typical_price.py
    tti.indicators package, _typical_price.py module unit tests.
"""
...
class TestTypicalPrice(unittest.TestCase, TestIndicatorsCommon):
    indicator = tti.indicators.TypicalPrice

    ti_data_rows = [0, 199, 200]

    df = pd.read_csv(
        "./data/sample_data.csv", parse_dates=True, date_format="%Y-%m-%d", index_col=0
    )

    indicator_input_arguments = {"period": 200}

    indicator_other_input_arguments = [{"period": 1}, {"period": 3169}, {"period": 5000}]
