...
class TestIntradayMovementIndex(unittest.TestCase):
    ...
    def test_input_data_required_column_close_missing(self):
        ...
        with self.assertRaises(ValueError):
            IntradayMovementIndex(pd.DataFrame(df.drop(columns=['close'])))

    def test_input_data_required_column_open_missing(self):
        df = pd.read_csv('./data/sample_data.csv', parse_dates=True,
                         index_col=0)

        with self.assertRaises(ValueError):
            IntradayMovementIndex(pd.DataFrame(df.drop(columns=['open'])))
    ...
    def test_input_data_empty(self):
        ...
        with self.assertRaises(ValueError):
            IntradayMovementIndex(pd.DataFrame(df[df.index >= '2032-01-01']))
    ...
    def test_period_parameter_wrong_type(self):
        ...
        with self.assertRaises(WrongTypeForInputParameter):
            IntradayMovementIndex(df, period='30')
    ...
    def test_period_parameter_wrong_value(self):
        ...
        with self.assertRaises(WrongValueForInputParameter):
            IntradayMovementIndex(df, period=0)
    ...
    def test_fill_missing_values_parameter_wrong_type(self):
        ...
        with self.assertRaises(WrongTypeForInputParameter):
            IntradayMovementIndex(df, period=200, fill_missing_values=1)
    ...
    def test_fill_missing_values_is_true(self):
        ...
        df_expected_result = pd.read_csv('./data/missing_values_filled.csv',
                                         parse_dates=True,
                                         index_col=0)[['open', 'close']]

        df_result = IntradayMovementIndex(df, period=20,
    ...
    def test_fill_missing_values_is_false(self):
        ...
        df_result = IntradayMovementIndex(df, fill_missing_values=False)\
            ._input_data
    ...
    def test_fill_missing_values_is_default_true(self):
        ...
        df_result = IntradayMovementIndex(df, period=20)._input_data
    ...
    def test_validate_indicator_one_row(self):
        ...
        with self.assertRaises(NotEnoughInputData):
            IntradayMovementIndex(df[df.index == '2000-02-01'])
    ...
    def test_validate_indicator_less_than_required_data(self):
        ...
        with self.assertRaises(NotEnoughInputData):
            IntradayMovementIndex(df.iloc[:13], period=14)
    ...
    def test_validate_indicator_full_data(self):
        ...
        df_expected_result = pd.read_csv(
            './data/test_intraday_movement_index_on_sample_data.csv',
            parse_dates=True,
            index_col=0).round(4)

        df_result = IntradayMovementIndex(df)._ti_data
    ...
    def test_getTiGraph(self):
        ...
        obv = IntradayMovementIndex(df)

        # Needs manual check of the produced graph
        self.assertEqual(obv.getTiGraph(), plt)

        obv.getTiGraph().savefig('./figures/test_intraday_movement_index.png')
        plt.close('all')
    ...
    def test_getTiValue_latest(self):
        ...
        self.assertEqual(list(df_expected_result.iloc[-1]),
                         IntradayMovementIndex(df).getTiValue())

    def test_getTiSignal(self):
        df = pd.read_csv('./data/sample_data.csv', parse_dates=True,
                         index_col=0)

        self.assertIn(IntradayMovementIndex(df).getTiSignal(),
                      [('buy', -1), ('hold', 0), ('sell', 1)])
