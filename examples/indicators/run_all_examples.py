...
def execute_example_code(
    indicator_object, output_file=None, add_info=None, figures_output_path=None, **kwargs
):
    """
    ...
    output_file_name = (
        figures_output_path + "example_" + type(indicator).__name__ + graph_name_suffix + ".png"
    )
    ...
    print("\n- Technical Indicator data:\n", indicator.getTiData(), file=output_file)

    # Get indicator's value for a specific date (randomly chosen)
    date = indicator.getTiData().index[random.randint(0, len(indicator.getTiData().index) - 1)]
    print(
        "\n- Technical Indicator value at ",
        date,
        " : ",
        indicator.getTiValue(date),
        sep="",
        file=output_file,
    )
    ...
    print(
        "\n- Technical Indicator value at",
        indicator.getTiData().index[-1],
        ":",
        indicator.getTiValue(),
        file=output_file,
    )

    # Get signal from indicator
    print("\n- Technical Indicator signal:", indicator.getTiSignal(), file=output_file)
