...
class ProjectionBands(TechnicalIndicator):
    ...
    def _calculateTi(self):
        ...
        if len(self._input_data.index) < self._period:
            raise NotEnoughInputData("Projection Bands", self._period, len(self._input_data.index))
        ...
        high_slope = (
            RollingOLS(
                endog=self._input_data["high"].fillna(value=0, inplace=False).to_list(),
        ...
        low_slope = (
            RollingOLS(
                endog=self._input_data["low"].fillna(value=0, inplace=False).to_list(),
                exog=sm.add_constant(list(range(len(self._input_data.index)))),
                window=self._period,
            )
            .fit(params_only=True)
    ...
    def getTiSignal(self):
        ...
        # distance
        if self._ti_data["upper_band"].iat[-1] - self._input_data["close"].iat[-1] < 0.15 * (
            self._ti_data["upper_band"].iat[-1] - self._ti_data["lower_band"].iat[-1]
        ):
            return TRADE_SIGNALS["sell"]

        # Price goes close to the lower band, closer than 15% of the bands
        # distance
        if self._input_data["close"].iat[-1] - self._ti_data["lower_band"].iat[-1] < 0.15 * (
            self._ti_data["upper_band"].iat[-1] - self._ti_data["lower_band"].iat[-1]
        ):
