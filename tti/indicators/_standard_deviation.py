...
class StandardDeviation(TechnicalIndicator):
    ...
    def _calculateTi(self):
        ...
        if len(self._input_data.index) < self._period:
            raise NotEnoughInputData(
                "Standard Deviation", self._period, len(self._input_data.index)
            )
    ...
    def getTiSignal(self):
        ...
        sma = (
            self._input_data.rolling(window=self._period, min_periods=self._period).mean().round(4)
        )

        # Price above average and volatility is high
        if self._input_data["close"].iat[-1] > sma.iat[-1, 0] and self._ti_data["sd"].iat[-1] > 2:
