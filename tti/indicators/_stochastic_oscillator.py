...
class StochasticOscillator(TechnicalIndicator):
    ...
    def __init__(
        ...
        if isinstance(k_periods, int):
            if k_periods > 0:
                ...
                raise WrongValueForInputParameter(k_periods, "k_periods", ">0")
        else:
            raise WrongTypeForInputParameter(type(k_periods), "k_periods", "int")

        if isinstance(k_slowing_periods, int):
            if k_slowing_periods in [1, 3]:
                self._k_slowing_periods = k_slowing_periods
            else:
                raise WrongValueForInputParameter(k_slowing_periods, "k_slowing_periods", "1 or 3")
        else:
            raise WrongTypeForInputParameter(type(k_slowing_periods), "k_slowing_periods", "int")

        if isinstance(d_periods, int):
            if d_periods > 0:
                self._d_periods = d_periods
            else:
                raise WrongValueForInputParameter(d_periods, "d_periods", ">0")
        else:
            raise WrongTypeForInputParameter(type(d_periods), "d_periods", "int")

        if isinstance(d_method, str):
            if d_method in ["simple", "exponential"]:
                self._d_method = d_method
            else:
                raise WrongValueForInputParameter(d_method, "d_method", "'simple' or 'exponential'")
        else:
    ...
    def _calculateTi(self):
        ...
        if len(self._input_data.index) < self._k_periods:
            raise NotEnoughInputData(
                "Stochastic Oscillator", self._k_periods, len(self._input_data.index)
            )
        ...
        so = pd.concat(
            [
                ...
                self._input_data["high"].rolling(self._k_periods).max(),
            ],
            axis=1,
        )

        columns_names = list(self._input_data.columns) + ["lowest_low", "highest_high"]
        ...
        so = pd.concat(
            [
                ...
                so["denominator"].rolling(self._k_slowing_periods).sum(),
            ],
            axis=1,
        )
        ...
        so = pd.concat(
            [
                so,
                so.rolling(
                    window=self._d_periods,
                    min_periods=self._d_periods,
    ...
    def getTiSignal(self):
        ...
        # then falls below 80.
        if (
            self._ti_data["%K"].iat[-2] > 80.0 > self._ti_data["%K"].iat[-1]
            or self._ti_data["%D"].iat[-2] > 80.0 > self._ti_data["%D"].iat[-1]
        ):
        ...
        # rises above 20.
        if (
            self._ti_data["%K"].iat[-2] < 20.0 < self._ti_data["%K"].iat[-1]
            or self._ti_data["%D"].iat[-2] < 20.0 < self._ti_data["%D"].iat[-1]
        ):
        ...
        if (
            self._ti_data["%K"].iat[-2] - self._ti_data["%K"].iat[-1]
            > 0.0
            > self._ti_data["%K"].iat[-1] - self._ti_data["%D"].iat[-1]
            and self._ti_data["%K"].iat[-1] > 80.0
        ):
            return TRADE_SIGNALS["sell"]
        ...
        if (
            ...
            < 0.0
            < self._ti_data["%K"].iat[-1] - self._ti_data["%D"].iat[-1]
            and self._ti_data["%K"].iat[-1] < 20.0
        ):
            return TRADE_SIGNALS["buy"]
