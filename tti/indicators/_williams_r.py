...
class WilliamsR(TechnicalIndicator):
    ...
    def _calculateTi(self):
        ...
        if len(self._input_data.index) < self._period:
            raise NotEnoughInputData("William's %R", self._period, len(self._input_data.index))

        wr = pd.DataFrame(index=self._input_data.index, columns=["wr"], data=None, dtype="float64")
        ...
        wr["wr"] = -100 * (
            (wr["highest_high"] - self._input_data["close"])
    ...
    def getTiSignal(self):
        ...
        # Oversold area is <= -80
        try:
            last_oversold_index = np.where(self._ti_data["wr"].iloc[:-1] <= -80)[0][-1]
        except:
            last_oversold_index = -1

        # Overbought area is >= -20
        try:
            last_overbought_index = np.where(self._ti_data["wr"].iloc[:-1] >= -20)[0][-1]
        except:
        ...
        if (last_oversold_index > last_overbought_index) and (
            self._ti_data["wr"].iat[-2] < -50 < self._ti_data["wr"].iat[-1]
        ):
